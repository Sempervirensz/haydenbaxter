...
| ETB-P2-03 | P2 | No resume or CV anywhere | **Fixed** | Unblocked: the PDF Hayden had to supply landed at `public/documents/Hayden-Baxter-Resume.pdf`, and the 2026 one-page version replaced it in `deb0c83`. The CTA had already flipped to "View resume" on its own, as the original note predicted. `5cc098b` then added **`/resume`**, a preview + download page, and repointed `RESUME_HREF` at it so every human entry point reaches a page rather than a raw file download; the file's own path is now `RESUME_FILE_HREF`. **The preview is a rendered image, not the PDF** — every response carries `X-Frame-Options: DENY` and `frame-ancestors 'none'`, so `<iframe>`/`<object>` of the PDF are refused, and embedding it would have required relaxing that hardening site-wide. `vercel.json` is unchanged. The image is regenerated by `npm run gen:resume-preview`, and `check:resume-preview` (wired into `npm run check`) fails the build if the PDF changes without it, so the page cannot silently serve a picture of an old resume. Evidence so far: `npm run check` and `next build` exit 0; `/resume` renders and the preview is legible in the Chromium preview browser **and real Chrome** at 1280x900, and at 375x812 where the image is the only preview a phone can show at all (iOS refuses inline PDFs); 0px horizontal overflow. **Not yet Verified: the cross-browser matrix has not been run.** |
