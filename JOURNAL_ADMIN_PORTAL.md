...
- `src/app/api/admin/finalize/route.dev.ts`  
  Dev-only finalization endpoint. Stages `src/data/journal.ts` and `public/images/blog`, runs `npm run check`, commits, and pushes the current branch.
...
8. Click `Final commit & push` when ready.
...
- `Final commit & push` stages only `src/data/journal.ts` and `public/images/blog`.
...
`Final commit & push` pushes the current local branch. If production only deploys from `main`, the pushed branch still needs to be merged or the finalization flow should be adjusted to target the production branch explicitly.

There are unrelated existing dirty files in the worktree (`src/components/work/work-together.css` and `src/styles/work-details.css`). The Journal finalization endpoint intentionally does not stage those files.
