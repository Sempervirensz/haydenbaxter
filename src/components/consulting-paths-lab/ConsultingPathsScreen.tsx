...
// The redesigned "Start a Consulting Project" answer — TWO paths, side by side.
//
// WHAT IT REPLACES
//
// Production answers this choice with one paper panel that lists AI Systems and
// Supply Chain as read-only blocks over a single generic "Discuss a project"
// button (WorkTogetherScreen + workTogether.ts → CONSULTING). Both disciplines
// are visible, but neither is actionable on its own.
//
...
//
// ONE MARKUP, FIVE DIRECTIONS
//
// Every art direction renders this exact DOM and this exact interaction — they
// differ in `data-layout`, `data-palette`, `data-surface` and `data-type` and
// in nothing else. That is deliberate: if the markup changed per direction the
// comparison would be between five prototypes rather than between five ways of
// drawing one idea. Anything a direction needs that is purely decorative (the
// ghost numeral, corner ticks, accent bands, numbered rows) is drawn in CSS.
...
export interface ScreenSkin {
  ...
  palette: PaletteId;
  surface: SurfaceId;
  /** null = the type scheme the direction was drawn for. */
  type: TypeSchemeId | null;
  /** null = the CTA recipe the direction was drawn for. */
  button: ButtonId | null;
}

export default function ConsultingPathsScreen({
  onBack,
  skin,
  /** Which path starts expanded. Used by the gallery to show the interaction
      without a click; null is the real first-run state. */
  initialOpen = null,
}: {
  onBack: () => void;
  skin: ScreenSkin;
  ...
  const [openId, setOpenId] = useState<ConsultingPathId | null>(initialOpen);
